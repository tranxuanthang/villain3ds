...
function choiceHandle(file_url , targetPath, cID, ContentCount, dldir){
				...
				$('#info'+cID+' #finished').on('click', function(){
					...
					if (fs.existsSync(targetPath)) {
						...
						if(fileSizeInBytes == fileSize){
							...
							$('#instruct'+cID).hide();
							document.querySelector('#p'+cID).MaterialProgress.setProgress(100);
							$('#done'+cID).html('Content #'+cID+' is successfully downloaded by user.');
							checkFinished(cID,ContentCount);
						} else {
	...
	req.on('error', function() {
        $('#info'+cID).html('Content #'+cID+' cannot be downloaded. Internet connection failed.<br><button id="dlanyway" class="mdl-button mdl-js-button mdl-button--primary">Redownload</button>');
		$('#info'+cID+' #dlanyway').on('click', function(){
			if (fs.existsSync(targetPath)) {fs.unlinkSync(targetPath);}
			choiceHandle(file_url , targetPath, cID, ContentCount, dldir);
		});
...
function downloadFile(file_url , targetPath, cID, ContentCount){
	...
	var stream = progress(request({
		...
		delay: 200,                       // Only start to emit after 1000ms delay, defaults to 0ms 
	...
	stream.on('finish', function () {
		...
		if (checkValid == true){
			$('#info'+cID).hide();
			document.querySelector('#p'+cID).MaterialProgress.setProgress(100);
			$('#done'+cID).html('Content #'+cID+' is successfully downloaded.');
			checkFinished(cID,ContentCount);
		} else {
			$('#done'+cID).html('');
			$('#info'+cID).html('Content #'+cID+' is somehow corrupted while transferring.<br><button id="redownload" class="mdl-button mdl-js-button mdl-button--primary">Redownload</button>');
			$('#info'+cID+' #redownload').on('click', function(){
				if (fs.existsSync(targetPath)) {fs.unlinkSync(targetPath);}
				choiceHandle(file_url , targetPath, cID, ContentCount, dldir);
			});
...
function showProgress(state, cID){
	...
	$('#done'+cID).html(percentage.toFixed(2) + '% | ' + humanFileSize(received,true) + ' out of ' + humanFileSize(total,false)
		+'<br>Speed: '+speed+'/s | Remaining time: '+ remainingTime
	);
...
function processDownloadTitle(results,data) {
	...
	tempfilename = sanitizefn(data.titleID+'.cia');
	filename = sanitizefn(data.name+' '+data.region+' ('+data.titleID+').cia');
	//}
}
function processDownload(event, receivedData){
	data = receivedData;
